...
class BiasFieldConfig(NamedTuple):
    """
...
@jax.jit
def get_bias_fields(points: jnp.ndarray, params: BiasFieldConfig) -> jnp.ndarray:
    bias = params.coil_factors * params.currents + params.stray_fields
    return jnp.broadcast_to(bias, (points.shape[0], 3))


def bias_field_params_to_dict(params: BiasFieldConfig) -> dict:
    # fmt: off
    return {
...
def bias_field_params_from_dict(data: dict) -> BiasFieldConfig:
    # fmt: off
    return BiasFieldConfig(
...
class BiasFields:
    ...
    def __init__(
        ...
        self.config = BiasFieldConfig(coil_factors, currents, stray_fields)

    def get_fields(self, points: jnp.ndarray) -> jnp.ndarray:
        return get_bias_fields(points, self.config)

    def to_dict(self) -> dict:
        return bias_field_params_to_dict(self.config)
